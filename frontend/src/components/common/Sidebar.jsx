...
const Sidebar = () => {
  ...
  const adminItems = [
    ...
    { path: "/admin/deals", label: "Deals", icon: "M9 5H7a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2V7a2 2 0 00-2-2h-2M9 5a2 2 0 002 2h2a2 2 0 002-2M9 5a2 2 0 012-2h2a2 2 0 012 2m-6 9l2 2 4-4" },
    ...
    { path: "/admin/audit-logs", label: "Audit", icon: "M9 5H7a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2V7a2 2 0 00-2-2h-2M9 5a2 2 0 002 2h2a2 2 0 002-2M9 5a2 2 0 012-2h2a2 2 0 012 2m-3 7h3m-3 4h3m-6-4h.01M9 16h.01" },
  ...
  return (
    <aside className="w-56 glass-panel mx-3 my-3 border border-border-subtle h-[calc(100vh-1.5rem)] flex flex-col relative shadow-2xl z-20 rounded-2xl bg-white/5 backdrop-blur-3xl">
      <div className="p-6">
        <button
          onClick={logout}
          className="w-full flex items-center space-x-2 text-primary-900 dark:text-primary-400 mb-6 hover:text-red-500 dark:hover:text-red-400 transition-colors group text-left"
        ...
        >
          <div className="w-8 h-8 rounded-lg bg-gradient-to-br from-primary-600 to-indigo-600 flex items-center justify-center shadow-lg shadow-primary-500/30">
            <span className="font-bold text-white text-sm">{isAdmin ? "A" : "S"}</span>
          </div>
          <span className="font-black text-sm tracking-wide uppercase flex-1 opacity-90">
            {isAdmin ? "Admin" : "Sales"}
          </span>
          <svg className="w-4 h-4 opacity-0 group-hover:opacity-100 transition-opacity" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M17 16l4-4m0 0l-4-4m4 4H7m6 4v1a3 3 0 01-3 3H6a3 3 0 01-3-3V7a3 3 0 013-3h4a3 3 0 013 3v1" /></svg>
        </button>
      </div>
      ...
      <nav className="flex-1 px-3 space-y-1 overflow-y-auto">
        {navItems.map((item) => (
          <NavLink
            ...
            className={({ isActive }) =>
              `group flex items-center space-x-3 px-3.5 py-2.5 text-sm font-medium rounded-xl transition-all duration-300 relative ${isActive
          ...
          >
            {({ isActive }) => (
              <>
                {isActive && (
                  <div className="absolute inset-0 bg-gradient-to-r from-primary-600 to-indigo-600 rounded-xl -z-10"></div>
                )}
                <svg className={`w-5 h-5 flex-shrink-0 transition-transform duration-300 ${isActive ? 'scale-110' : 'group-hover:scale-110'}`} fill="none" stroke="currentColor" viewBox="0 0 24 24">
      ...
      <div className="p-4 mt-auto">
        <div className="bg-gradient-to-b from-surface-2 to-surface-3 p-4 rounded-xl border border-border-subtle shadow-inner text-center">
          <div className="w-2 h-2 rounded-full bg-emerald-500 mx-auto mb-2 animate-pulse"></div>
          <p className="text-[10px] uppercase font-bold text-text-muted tracking-widest mb-1">Status: Online</p>
          <p className="text-[10px] text-text-muted opacity-50 font-mono">v2.5.0</p>
        </div>
      </div>
    </aside>
  );
