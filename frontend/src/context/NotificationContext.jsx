...
export const NotificationProvider = ({ children }) => {
  ...
  useEffect(() => {
    if (userId) {
      ...
      axios.get(`http://localhost:8080/notifications?userId=${userId}`)
        .then(res => {
          console.log(`Received ${res.data.length} notifications for user ${userId}`);
  ...
  }, [userId]); // Dependency on userId is more precise than [auth]
  ...
  const addNotification = useCallback(async (notification) => {
    ...
    try {
      const res = await axios.post("http://localhost:8080/notifications", {
        userId,
      ...
      setNotifications(prev => {
        ...
        if (isReadLocally) {
          axios.patch(`http://localhost:8080/notifications/${realNotification.id}/read`).catch(e => console.error("Sync read failed", e));
        }
